...
export const GatcWorkbench: React.FC = () => {
  ...
  const handleInspectionSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Failed to record laboratory calibration.');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8">
                      ...
                      <td className="py-4 px-6 text-right">
                        ...
                        {app.status === 'SUBMITTED' || app.status === 'ALLOCATED' ? (
                          ...
                          >
                            ...
                            <span>{issuingCertId === app.id ? 'Endorsing...' : 'Endorse & Issue Certificate'}</span>
                          </button>
                        ) : app.status === 'CERTIFIED' ? (
                          <div className="flex items-center justify-end gap-1.5">
                            {app.certificate && (
      ...
      {/* 2. Laboratory Calibration Record Modal */}
      {showInspectModal && selectedApp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
              <div>
                <h3 className="text-lg font-bold text-slate-900">Record Laboratory Calibration Observations</h3>
            ...
            <div className="flex items-center justify-between p-2.5 mb-4 bg-indigo-50/70 rounded-xl border border-indigo-200">
              ...
              <div className="flex gap-1.5">
                ...
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-rose-600 hover:bg-rose-700 text-white text-[11px] font-bold transition shadow-2xs"
                >
                  ✕ Exceeds MPE
                </button>
              </div>
            ...
            <form onSubmit={handleInspectionSubmit} className="space-y-4 text-xs font-medium">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-teal-600 text-white font-bold hover:bg-teal-700 shadow-sm cursor-pointer"
                >
                  {submittingInspection ? 'Recording Calibration...' : 'Record Test & Sign Observations'}
                </button>
              </div>
            </form>
          </div>
